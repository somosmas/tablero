@extends('backend', ['title' => 'Editar Meta | Tablero de control público de seguimiento del PA15.'])
...
<!--breadcrumb-->
			...
			<ol class="breadcrumb">
                ...
                <li class="active">
                    ...
                    @if($objective->step_num == 4) 
                      {{$step_num}}
                    @else
                      Meta {{$objective->event_num}} ({{$step_num}})
                    @endif
                </li>
            </ol>
		</div>
 ...
 <div class="row">
    ...
    @if($objective->step_num == 4) 
      {{$step_num}}
    @else
      Meta {{$objective->event_num}} <small>({{$step_num}})</small>
    @endif
  </h1>
  ...
  * - -   F O R M   C O N T E N T   - - *
  ...
  <div class="form-group">
    {{Form::label('title', 'Meta: ', array('class'=>'col-sm-2 control-label'))}}
	  <div class="col-sm-8">
  ...
  <div class="form-group">
    {{Form::label('description', 'Actividad', array('class'=>'col-sm-2 control-label'))}}
	  <div class="col-sm-8">
	  @if(Auth::user()->user_type == "society")
  ...
  <!--ADVANCE DESCRIPTION-->
  <div class="form-group">
    {{Form::label('advance_description', 'Sub-Actividades: ', array('class'=>'col-sm-2 control-label'))}}
	  <div class="col-sm-8">
	  	@if(Auth::user()->user_type == "society")
...
<!-- AQUÍ TERMINA LO NUEVO -->
  ...
  <div class="form-group">
    ...
    {{ $objective->status == "b" ? link_to('objective/conclude/' . $objective->id, 'Finalizar Meta >', array("class"=>"btn btn-lg btn-success")) : ""}}
	</div>
  </div>
